...
public class Piece {
	...
	*/
	...
	//	this.body = Arrays.copyOf(points, points.length);
		...
		Arrays.fill(sk, height);
		for(TPoint p : points){
			if(sk[p.x]>p.y) sk[p.x]=p.y;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		// YOUR CODE HERE
		root.next=recMFR(root,root);
		return root; // YOUR CODE HERE
	}
	
	private static Piece recMFR(Piece r, Piece next){
		Piece cur = next.computeNextRotation();
		if(cur.equals(r)) return r;
		else{
			cur.next=recMFR(r,cur);
